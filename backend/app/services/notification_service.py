...
def trigger_notification(event, user_id, order_number=None, status=None, note=None):
    """
    Triggers an in-app notification for order checkout/status changes.
    """
    from app.db import get_db
    from bson import ObjectId
    db_conn = get_db()
    
    # 1. Resolve order_id from order_number if provided
    order_id = None
    if order_number:
        order = db_conn.orders.find_one({"order_number": order_number})
        if order:
            order_id = order.get("_id")
            
    # 2. Build user-facing message
    if event == "order_created":
        message = f"Your order #{order_number} has been successfully created."
    elif event == "order_status_changed":
        message = f"Your order #{order_number} status has been updated to '{status}'."
        if note:
            message += f" Note: {note}"
    else:
        message = f"Order #{order_number} notification: {event}."
        
    # 3. Create database entry for in-app notification feed
    create_notification(user_id, message, order_id=order_id)
    
    # 4. Attempt to notify user via channels (Email/Telegram) using notify_user
    try:
        user = db_conn.users.find_one({"_id": ObjectId(user_id) if isinstance(user_id, str) else user_id})
        if user:
            recipient = {
                "user_id": str(user_id),
        ...
        logger.error(f"Failed to dispatch notifications to external channels: {e}")
