...
@ai_blueprint.route('/generate-preview', methods=['POST'])
def handle_generate_preview():
    ...
    if not user_id and not session_id:
        return jsonify({
            ...
            "errors": {"session_id": "session_id is required for guest users."}
        }), 400
    ...
    if selected_attributes is None:
        return jsonify({
            ...
            "errors": {"selected_attributes": "selected_attributes dictionary is required."}
        }), 400
...
@ai_blueprint.route('/generations/<user_id>', methods=['GET'])
@auth_required
def handle_get_user_history(user_id):
    """Retrieves paginated, date-descending successful generations for a user (auth-gated)."""
    if g.user_id != user_id and g.current_user.get("role") != "admin":
        return jsonify({
            "success": False,
            "message": "Access denied. Mismatched user identity."
        }), 403
