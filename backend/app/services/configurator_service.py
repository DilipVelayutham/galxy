...
def validate_attributes_by_id(category_id, selected_attributes):
    """
    Wrapper that resolves category from DB by ID/string and validates attributes.
    """
    ...
    res = validate_attributes(category, selected_attributes)
    if not res["valid"]:
        # Put the first error message as reason
        first_error_key = next(iter(res["errors"].keys()))
        first_error_msg = res["errors"][first_error_key]
        res["reason"] = f"{first_error_key}: {first_error_msg}"
