...
def generate_unique_slug(name: str, collection: Collection, category_id: Optional[ObjectId] = None) -> str:
    from slugify import slugify as py_slugify
    base_slug = py_slugify(name, allow_unicode=False, separator="-")
    if not base_slug:
        base_slug = "category"
    ...
    while True:
        ...
        if category_id:
            try:
                ...
                query["_id"] = {"$ne": cat_obj_id}
            except Exception:
                query["_id"] = {"$ne": category_id}
