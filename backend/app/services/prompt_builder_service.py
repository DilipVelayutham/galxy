...
def sanitize_custom_text(text):
    ...
    injection_patterns = [
        ...
        r"disregard\s+your\s+rules", r"disregard\s+rules", r"forget\s+your\s+instructions"
    ]
    ...
    # 4. Collapse runs of 4+ identical special/punctuation characters to 1
    text = re.sub(r'([^\w\s])\1{3,}', r'\1', text)
    
    # 5. Filter down to safe characters: alphanumeric, spaces, quotes, hyphens, basic punctuation
...
def build_prompt(category, selected_attributes):
    ...
    if not template:
        raise ValueError("no ai_prompt_template")
        
    schema_list = category.get("attribute_schema") or category.get("attributes") or []
...
_sanitize_text = sanitize_custom_text
