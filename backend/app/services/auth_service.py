...
class AuthService:
    ...
    def signup(name, email, phone, password):
        try:
            user_data, access_token, refresh_token = AuthService.signup_user(name, email, phone, password)
            return {
                "success": True,
                "data": {
                    "user": user_data,
            ...
            return {"success": False, "message": e.message, "errors": e.errors}, e.status_code

    @staticmethod
    def login(email, password):
        try:
            user_data, access_token, refresh_token = AuthService.login_user(email, password)
            return {
                "success": True,
                "data": {
                    "user": user_data,
    ...
    @staticmethod
    def generate_access_token(user_id, email=None, role="customer"):
        from app.utils.token_helper import generate_access_token as gen_token
        return gen_token(user_id, role)
